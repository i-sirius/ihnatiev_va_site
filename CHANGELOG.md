...
- **Activity3 audio update:**
  - Піднято блок аудіозаписів проповідей вище блоку `Фото` на сторінці духовної діяльності.
  - Прибрано службовий підпис `тестовий формат` / `test format`.
  - Додано новий аудіозапис проповіді про двох сліпих і німого біснуватого.
  - Додано production `.m4a` файл для нового аудіозапису.
  - Оновлено `audio.json` і search index; кількість audio-записів у пошуку — 3.

## 0.7.80.290626

- **Deep site search:**
  - Додано глибшу індексацію сторінок, розділів, наукових праць, завантажень, аудіо, відео та супровідних PDF-матеріалів.
  - Додано локальні search metadata для відео та секцій сайту.
  - Покращено ранжування результатів: title/tags/keywords мають вищу вагу, службові слова не перебивають релевантність.
  - Виправлено обробку коротких службових слів (`за`, `на`, `не` тощо) та підсвічування збігів усередині довших слів.
  - Виправлено появу `[object Object]` у metadata результатів.
  - Додано локалізовані action labels для результатів пошуку: `Перейти`, `Переглянути`, `Прослухати`, `Відкрити матеріал`, `Перейти до списку`.
  - Покращено переходи з пошуку до конкретних відео, публікацій і завантажень після асинхронного рендерингу сторінок.
  - Покращено пошук у завантаженнях за українськими й англійськими назвами незалежно від поточної мови інтерфейсу.
  - Додано fallback-рендер відеокарток із локального `video-index.json`, якщо зовнішнє джерело відео недоступне.
...
- **Video support documents:**
  - Додано підтримку супровідних документів для окремих відео на сторінці “Наукова діяльність”.
  - Thumbnail відео лишається клікабельним і веде на YouTube.
  - Основна кнопка `Дивитись` лишається головною дією.
  - Додано опційну secondary-кнопку `Матеріали` / `Materials` для PDF/DOCX матеріалів.
  - Додано перші PDF-матеріали для відео про ісихазм і часте Причастя та про тілесність і синергію.
  - Оновлено оформлення іконки кнопки `Матеріали`.
